...
class LastFragment : Fragment() {
    ...
    override fun onCreateView(
       ...
       val view:View=inflater.inflate(R.layout.fragment_last, container, false)
        ...
        rv=view.findViewById(R.id.last_rv)
        rv.setHasFixedSize(true)
        rv.layoutManager=GridLayoutManager(requireContext(),2)
        rv.adapter=adapter
        root.child("last").addValueEventListener(object :ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
               arr.clear()
                for(imagesnapshot in snapshot.children){
                    val ig=imagesnapshot.child("filmy").value.toString()
                    ...
                    arr.add(image(ig,video,descr,dire,act_img,act_name,act_img2,act_name2,act_img3,act_name3,act_img4,act_name4,film_name,rate,chapter,type))
                }
